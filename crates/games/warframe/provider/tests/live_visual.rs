...
fn current_game_screens_and_relic_picker() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let demand: Vec<_> = provider
        ...
        .capabilities
        .iter()
        .filter(|cap| matches!(cap.topic, "warframe.screens" | "warframe.relic_rewards"))
        .collect();
    ...
    for second in 0..2 {
        ...
        assert_eq!(events.snapshots.len(), 2, "health: {:?}", health.health);
        for (topic, value) in events.snapshots {
            if topic == "warframe.screens" {
                println!("screens: {}", value["screens"]);
            } else {
                println!("relic picker: {}", value["picker"]);
            }
        }
        println!(
