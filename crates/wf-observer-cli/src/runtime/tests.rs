...
#[test]
fn legacy_agent_remains_visible_but_cannot_be_reused_for_either_mode() -> anyhow::Result<()> {
    let mut legacy = serde_json::to_value(record(current_process()?, None))?;
    ...
    assert!(String::from_utf8(output)?.contains("Active access: unknown"));
    Ok(())
}
...
fn service_liveness_is_independent_of_its_sessions() -> anyhow::Result<()> {
    ...
    for (target, target_status) in [
        ...
        status::write_status(&mut status, &actual, AccessMode::Local)?;
