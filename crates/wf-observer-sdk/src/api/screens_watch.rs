//! Typed screens reads and watches. State/lifetimes belong to the Rust SDK.
use crate::api::{ObserverError, UnavailableReason, WarframeScreens, runtime};
use crate::warframe::ScreensTopic;
use std::{sync::Arc, time::Duration};

#[boltffi::data]
#[derive(Debug, Clone, ..Eq)]
pub enum ScreensState {
    Waiting,
    Ready { value: WarframeScreens },
    Unavailable { reason: UnavailableReason },
}
impl From<crate::raw::State<warframe_model::ScreensSnapshot>> for ScreensState {
    fn from(state: crate::raw::State<warframe_model::ScreensSnapshot>) -> Self {
        match state {
...
pub struct ScreensCapability {
    inner: crate::raw::Capability<ScreensTopic>,
}
impl ScreensCapability {
    pub(crate) fn new(inner: crate::raw::Capability<ScreensTopic>) -> Self {
        Self { inner }
    }
}
#[boltffi::export]
impl ScreensCapability {
    ...
    pub async fn read(&self) -> Result<WarframeScreens, ObserverError> {
    ...
    pub async fn read_with_timeout(
        &self,
        timeout_ms: u32,
    ) -> Result<WarframeScreens, ObserverError> {
    ...
    pub async fn cached(&self) -> Result<Option<WarframeScreens>, ObserverError> {
        let capability = self.inner.clone();
    ...
    pub async fn watch(&self) -> Result<ScreensWatch, ObserverError> {
        let capability = self.inner.clone();
        runtime::execute(async move { capability.watch().await })
            .await?
            .map(|inner| ScreensWatch {
                inner: Arc::new(inner),
            })
            .map_err(Into::into)
    }
...
pub struct ScreensWatch {
    inner: Arc<crate::raw::SnapshotWatch<ScreensTopic>>,
}
#[boltffi::export]
impl ScreensWatch {
    ...
    pub fn current(&self) -> Result<ScreensState, ObserverError> {
    ...
    pub async fn next(&self) -> Result<Option<ScreensState>, ObserverError> {
